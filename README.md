...
在 Cloudflare 控制台创建 D1 数据库，记下 database_name 并在 wrangler.toml 中绑定为 MUSIC_D1。
在 Cloudflare KV 中创建命名空间并在 Pages & Worker 中绑定为 MUSIC_KV（可选，但推荐）。
在 Cloudflare 仪表盘创建 Worker，或使用 Wrangler 部署（我们在仓库中包含 wrangler.toml）。
在项目的 Pages 设置中添加环境变量 API_BASE_URL，值为你的 Worker 域名（例如 https://your-worker.YOUR_DOMAIN.workers.dev）。
本地开发
...
工作流文件：.github/workflows/deploy.yml（已包含在仓库），用于在 push 到 main 时自动部署 Worker（wrangler）和 Pages（pages-action）。
前端配置
